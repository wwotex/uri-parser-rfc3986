...
inline void test_uri(std::string_view test_case) {
    ...
    std::printf("full: \t\t\t%s (%s)\n", uri.m_uri.data(), uri.has_error ? "ERROR" : "VALID");
    ...
    std::cout << "query:        \t" << uri.m_query << "\n";
    std::cout << "fragment:     \t" << uri.m_fragment << "\n";
...
int main() {
    ...
    test_uri("x://mambo:1223/p?Hello%20World%21#a%3Ab%2Fc%3F%7E");
}
